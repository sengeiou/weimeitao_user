 ...
 */
public class ForumMeListAdapter extends BaseQuickAdapter<ForumListBean.DataBean.RecordsBean, BaseViewHolder> {
    private ForumMeActivity meActivity;

    public ForumMeListAdapter(List<ForumListBean.DataBean.RecordsBean> data, ForumMeActivity meActivity) {
    ...
    protected void convert(BaseViewHolder helper, ForumListBean.DataBean.RecordsBean item) {
        ...
        boolean isPreEdit = "pending".equals(meActivity.operationType);//是否为审核
        helper.setText(R.id.tvName, StringUtils.init().fixNullStr(item.createUser))
                ...
                .setVisible(R.id.tvType, isPreEdit)
                .setVisible(R.id.flReviewForum, isPreEdit)
                .setText(R.id.tvReview, "3".equals(item.auditStatus) ? "审核中" : "审核失败")
                .setText(R.id.tvTitle, StringUtils.init().fixNullStr(item.content))
                .addOnClickListener(R.id.llZan)
                .addOnClickListener(R.id.ivDelForum);
    }
}
