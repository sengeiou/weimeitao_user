...
import com.wmtc.wmt.forum.presenter.ForumEditHejiPresenter;
 ...
 */
public class ForumHejiEditActivity extends SuperBaseActivity implements AMapLocationListener, PoiSearch.OnPoiSearchListener {
    ...
    private ForumEditHejiPresenter mPresenter;
    ...
    public void initRootData(View view) {
        ...
        mSendPojo.title = mDataBean.title;
        if (mDataBean.pictures != null && mDataBean.pictures.size() > 0) {
            mSendPojo.bannerUrl = mDataBean.pictures.get(0);
        }
        mSendPojo.topicList = mDataBean.topicListStr;
        mEditor.setHtml(mDataBean.content);
        mTvHot.setText(mDataBean.topicListStr);
        mEditTitle.setText(mDataBean.title);
        requestLocation();
        EventBus.getDefault().register(this);
        mPresenter = new ForumEditHejiPresenter(this);
        mPresenter.ossToken();
        Glide.with(mActivity).load(WmtApplication.url_host + mSendPojo.bannerUrl)
...
//        mEditor.setPlaceholder("分享您的生美小常识 比如有趣的护肤体验，新奇有效的护肤小秘诀...");
        ...
        mIvBanner.setOnClickListener(v -> {
            AndPermission.with(this)
                    .permission(Permission.CAMERA, Permission.WRITE_EXTERNAL_STORAGE)
                    .onGranted(permissions -> CameraUtil.getInstance().openSingalCamerNoCrop(mActivity, 199))
                    .onDenied(permissions -> AndPermission.hasAlwaysDeniedPermission(mActivity, permissions))
                    .start();
        });
    }
    ...
    private void saveSend(String status) {
        ...
        if (mBannerFile != null) {
            ...
            if (mType.equals("heji")) {
                mPresenter.editHeji(mSendPojo);
            } else {
                mSendPojo.orderId = mBundle.getString("orderId");
                mPresenter.editRiji(mSendPojo);
            }
        }
    }
    ...
    public ArrayList<File> mArrayListBanner = new ArrayList<>();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (mArrayListBanner != null) {
            mArrayListBanner.clear();
        }
        ...
        if (requestCode == 199 && resultCode == RESULT_OK && data != null) {
            ...
            Observable.just(pathList).subscribeOn(Schedulers.io()).map(strings -> {
                ...
                isZiped = true;
                size = 0;
                mBannerFile = files.get(0);
                Glide.with(mActivity).load(mBannerFile).into(mIvBanner);
            });
        }
    }
