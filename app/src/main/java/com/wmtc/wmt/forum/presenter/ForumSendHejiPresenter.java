 ...
 */
public class ForumSendHejiPresenter extends BasePresenter<ForumHejiSendActivity> {

    private final CommonModel mModel;

    public ForumSendHejiPresenter(ForumHejiSendActivity iView) {
        super(iView);
    ...
    public void saveRiji(SendPojo pojo) {
        mModel.saveRiji(pojo).subscribe(new DefaultCallBackObserver<BaseBean>(this) {
            @Override
            public void responseSuccess(BaseBean baseBean) {
                mIView.sendOk();
            }
