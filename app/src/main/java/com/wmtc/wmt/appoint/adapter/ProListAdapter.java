 ...
 */
public class ProListAdapter extends TagAdapter<ShopProjectNameBean.DataBean> {
    public ProListAdapter(List<ShopProjectNameBean.DataBean> datas) {
        super(datas);
    }

    @Override
    public View getView(FlowLayout parent, int position, ShopProjectNameBean.DataBean bean) {
        TextView textView = (TextView) LayoutInflater.from(parent.getContext()).inflate(R.layout.adapter_dict, parent, false);
        textView.setText(bean.dictProjectName);
        return textView;
    }
}
