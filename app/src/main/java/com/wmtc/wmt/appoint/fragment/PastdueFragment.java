...
import com.wmtc.wmt.appoint.presenter.PastduePresenter;
import com.wmtc.wmt.forum.adapter.PastduedAdapter;

import top.jplayer.baseprolibrary.ui.fragment.SuperBaseFragment;

public class PastdueFragment extends SuperBaseFragment {

    private PastduedAdapter mAdapter;
    private PastduePresenter mPresenter;
    private String type = "1";

    @Override
    public int initLayout() {
        return R.layout.activity_past_due;
    }

    @Override
    protected void initData(View rootView) {
        initRefreshStatusView(rootView);
        mPresenter = new PastduePresenter(this);
        mAdapter = new PastduedAdapter(null);
        mRecyclerView.setAdapter(mAdapter);
        RedBagHistoryActivity activity = (RedBagHistoryActivity) mActivity;
        type = activity.type;
        KLog.e("TAG", "已过期-获得到的type" + type);
        if ("优惠劵历史".equals(type)) {
            this.type = "1";
        } else {
            this.type = "2";
        }
        getCouponList(type, "", "3");
    }
