...
import com.zhaoss.weixinrecorded.util.UriUtils;
...
public class RecordedActivity extends BaseActivity {
    ...
    private ImageView mIv_camera_sel;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        LanSongFileUtil.setFileDir(Environment.getExternalStorageDirectory().getPath() + "/wmt/");
    ...
    private void initUI() {
        ...
        mIv_camera_sel = findViewById(R.id.iv_camera_sel);
        ...
        mIv_camera_sel.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    public String h264ToMp4() {
        ...
        for (int x = 0; x < segmentList.size(); x++) {
            String tsPath = LanSongFileUtil.DEFAULT_DIR + System.currentTimeMillis() + ".mp4";
            mVideoEditor.h264ToMp4(segmentList.get(x), tsPath);
            tsList.add(tsPath);
        }
        for (String s : tsList) {
            Log.e("oooo", s + "-----------------");
        ...
        String mp4Path = mVideoEditor.executeConcatMP4(tsList.toArray(new String[]{}));
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == RESULT_OK && data != null) {
            if (requestCode == REQUEST_CODE_KEY) {
                ...
                closeProgressDialog();
                Intent intent = new Intent(mContext, EditVideoActivity.class);
                intent.putExtra(INTENT_PATH, UriUtils.getPath(mContext, uri));
                startActivityForResult(intent, REQUEST_CODE_KEY);
            }
        } else {
            cleanRecord();
